...
/***
 * Represents a real number using a double-precision floating point value in the
 * logrithmic scale, providing extremely high dynamic range in arithmetic
 * operations
 *
...
public class LogNumber extends Number implements Comparable<LogNumber> {
  ...
  public LogNumber divide(final LogNumber right) {
    if (null == right)
      return this;
    if (0 == right.type)
      return new LogNumber(this.type, Double.NaN);
  ...
  public LogNumber inverse() {
    return new LogNumber(this.type, -this.logValue);
  }
