...
public class MutationTrainer {
  ...
  private void align(TrainingContext trainingContext, List<DynamicRateTrainer> population) {
    List<List<List<double[]>>> signatures = population.stream().map(t -> {
    ...
    int canonicalIndex = IntStream.range(0, population.size()) //
    ...
    List<PermutationLayer> syncLayers = population.get(canonicalIndex).getGradientDescentTrainer().getNet()
        .getChildren().stream().filter(x -> x instanceof PermutationLayer).map(x -> (PermutationLayer) x)
        .collect(Collectors.toList());
    ...
    IntStream.range(0, signatures.size()).filter(i -> i != canonicalIndex).forEach(individual -> {
      IntStream.range(0, syncLayers.size()).forEach(layerIndex -> {
        List<double[]> canonicalSignature = signatures.get(canonicalIndex).get(layerIndex);
        List<double[]> individualSignature = signatures.get(individual).get(layerIndex);
        assert (canonicalSignature.size() == individualSignature.size());
        if (0 < canonicalSignature.size()) {
          List<Tuple2<Integer, Integer>> permute = findMapping(individualSignature, canonicalSignature);
          log.debug(String.format("Permutation in layer %s from %s to %s: %s", layerIndex, individual, canonicalIndex, permute));
          DAGNetwork net = population.get(individual).getGradientDescentTrainer().getNet();
  ...
  public static List<Tuple2<Integer, Integer>> findMapping(List<double[]> from, List<double[]> to) {
    int dim = from.get(0).length;
    assert (from.stream().allMatch(x -> dim == x.length));
    assert (to.stream().allMatch(x -> dim == x.length));

    double[][] vectorsFrom = IntStream.range(0, dim).mapToObj(i -> from.stream().mapToDouble(x -> x[i]).toArray()).toArray(i -> new double[i][]);
    double[][] vectorsTo = IntStream.range(0, dim).mapToObj(i -> to.stream().mapToDouble(x -> x[i]).toArray()).toArray(i -> new double[i][]);
    ...
    IntStream.range(0, dim).mapToObj(fromIndex -> {
      return IntStream.range(0, dim).mapToDouble(toIndex -> {
        ...
        double magFrom = 0;
        double magTo = 0;
        double[] vfrom = vectorsFrom[fromIndex];
        double[] vto = vectorsTo[toIndex];
        for (int k = 0; k < vfrom.length; k++) {
          double vkfrom = vfrom[k];
          double vkto = vto[k];
          dot += vkfrom * vkto;
    ...
    for (int fromIndex = 0; fromIndex < dim; fromIndex++) {
      ...
      int toIndex = toIndexes.stream().filter(x -> toIndexes.contains(x)) //
