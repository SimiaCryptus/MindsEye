...
public abstract class MappedSynapseLayer extends NNLayer {
    ...
    public MappedSynapseLayer setWeights(final java.util.function.ToDoubleFunction<Coordinate> f) {
      ...
      private void learn(final Tensor[] delta, final DeltaSet buffer) {
        ...
        DeltaBuffer deltaBuffer = buffer.get(MappedSynapseLayer.this, getWeights());
          ...
          java.util.stream.IntStream.range(0, threads).parallel().forEach(thread -> {
              ...
              java.util.stream.IntStream.range(0, result.data.length).filter(i -> thread == (i % threads)).forEach(dataIndex -> {
                  final double[] deltaData = delta[dataIndex].getData();
