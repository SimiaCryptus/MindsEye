...
public class GradientDescentBuffer {
  public static boolean DEBUG = false;
  private static final Logger log = LoggerFactory.getLogger(GradientDescentBuffer.class);
  ...
  protected GradientDescentBuffer() {
    super();
    this.minInversionRatio = 0;
    this.sink = null;
  }
  
  public GradientDescentBuffer(final double minInversionRatio, final DeltaSink sink) {
    this.sink = sink;
  ...
  public synchronized void feed(final NDArray weightGradient, final double[] data) {
    ...
    if (GradientDescentBuffer.DEBUG) {
      GradientDescentBuffer.log.debug(String.format("Input: %s & %s", weightGradient, Arrays.toString(data)));
    }
    if (0 == this.bufferPos) {
      ...
      if (GradientDescentBuffer.DEBUG) {
        GradientDescentBuffer.log.debug(String.format("Initialized with %s rows for %s weights and %s signal values", endx, inx, outx));
      }
    ...
    if (this.bufferPos >= this.gradientBuffer.getDims()[1]) {
      ...
      boolean gradDesc = true;
      if (gradDesc) {
        DoubleMatrix matrixA = new DoubleMatrix(this.signalBuffer.length, 1, this.signalBuffer);
        DoubleMatrix matrixB = new DoubleMatrix(gradient.getDims()[0], gradient.getDims()[1], gradient.data);
        inverted = matrixB.mmul(matrixA).data;
        //inverted = matrixA.transpose().mmul(matrixB).data;
      } else {
        inverted = org.jblas.Solve.solveLeastSquares(
      ...
      if (GradientDescentBuffer.DEBUG) {
        GradientDescentBuffer.log.debug(String.format("Processing feedback inversion to produce deltas: %s", Arrays.toString(inverted)));
      }
