...
public class NumberVector implements VectorLogic<NumberVector> {
  ...
  public <T extends Number> NumberVector(T[] values) {
    this.array = Arrays.stream(values).mapToDouble(x->x.doubleValue()).toArray();
  }

  public <T extends Number> NumberVector(double[] values) {
  ...
  protected <T extends Number> NumberVector join(final NumberVector right, final DoubleBinaryOperator joiner) {
    return new NumberVector(IntStream.range(0, this.getArray().length).mapToDouble(i -> {
      final double l = this.getArray()[i];
      final double r = right.getArray()[i];
      if (Double.isFinite(l) && Double.isFinite(r)) return joiner.applyAsDouble(l, r);
      if (Double.isFinite(l)) return r;
      if (Double.isFinite(r)) return l;
      return Double.NaN;
    }).toArray());
  }
  ...
  @Override
  public double l1() {
    return Math.sqrt(Arrays.stream(this.getArray()).map(v -> v).sum());
  }
  
  @Override
  public double l2() {
    return Math.sqrt(Arrays.stream(this.getArray()).map(v -> v * v).sum());
  }
  
  protected NumberVector map(final DoubleUnaryOperator mapper) {
    return new NumberVector(Arrays.stream(this.getArray()).map(x -> mapper.applyAsDouble(x)).toArray());
  }
  ...
  protected double combine(final NumberVector right, final DoubleBinaryOperator joiner) {
    return IntStream.range(0, this.getArray().length).mapToDouble(i -> {
      final double l = this.getArray()[i];
      final double r = right.getArray()[i];
  ...
  public String toString() {
    ...
    builder.append(Arrays.toString(this.getArray()));
    builder.append("]");
