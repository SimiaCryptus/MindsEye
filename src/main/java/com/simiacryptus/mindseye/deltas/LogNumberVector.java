...
public class LogNumberVector implements VectorLogic<LogNumberVector> {
  ...
  protected <T extends LogNumber> LogNumberVector join(final LogNumberVector right, final BiFunction<T, T, T> joiner) {
    return new LogNumberVector(IntStream.range(0, this.getArray().length).mapToObj(i -> {
      final T l = (T) this.getArray()[i];
  ...
  @Override
  public double l1() {
    return Math.sqrt(Arrays.stream(this.getArray()).mapToDouble(v -> {
  ...
  protected <T extends LogNumber> LogNumberVector map(final Function<T, T> mapper) {
    return new LogNumberVector(Arrays.stream(this.getArray()).map(x -> mapper.apply((T) x)).toArray(i -> new LogNumber[i]));
  }
