...
public class UnivariateOptimizer {
  ...
  public List<PointValuePair> getKeyPoints() {
    ...
    final PointValuePair bottomLeft = this.points.stream().filter(x -> x.getFirst()[0] < bottom.getFirst()[0]).max(Comparator.comparing(x -> x.getFirst()[0]))
        .orElseThrow(() -> new RuntimeException());
    if (bottom == bottomLeft)
      return this.points;
    final PointValuePair bottomRight = this.points.stream().filter(x -> x.getFirst()[0] > bottom.getFirst()[0]).min(Comparator.comparing(x -> x.getFirst()[0]))
        .orElseThrow(() -> new RuntimeException());
    final List<PointValuePair> newList = Stream.of(new PointValuePair[] { bottomLeft, bottom, bottomRight }).distinct().collect(Collectors.toList());
    assert newList.size() == 3;
    if (isVerbose()) {
      UnivariateOptimizer.log.debug(newList.stream().map(pt -> String.format("%s=%s", Arrays.toString(pt.getFirst()), pt.getSecond())).reduce((aa, bb) -> aa + ", " + bb).get());
    }
    return newList;
  }
  ...
  public synchronized PointValuePair minimize(final double start) {
    ...
    final UnivariatePointValuePair optim = new BrentOptimizer(1e-2, 1e-6).optimize(GoalType.MINIMIZE, new UnivariateObjectiveFunction(this.f),
        new SearchInterval(leftX, rightX, midX), new MaxEval(100));
